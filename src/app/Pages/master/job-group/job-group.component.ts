...
export class JobGroupComponent implements OnInit {
  jobGroupForm: FormGroup; flag; pkey: number = 0;
  ...
  ngOnInit(): void {
    this.jobGroupForm = this.fb.group({
      ...
      directCompletion:['', [Validators.required]]
    });
    //this.jobGroupForm.controls.directCompletion.setValue('');
    this.loadRights();
    this.loadData(0);
  }
  get fm() { return this.jobGroupForm.controls };
...
console.log(error);
  ...
  Updatedata(id) {
    ...
    this.unitmasterService.getJobGroupById(id)
      .subscribe((response) => {
        if (response.status) {
          this.jobGroupForm.patchValue(response.data);
  ...
  clear() {
    this.jobGroupForm.reset();
    this.jobGroupForm.controls.jobGroupId.setValue(0);
    this.jobGroupForm.controls.directCompletion.setValue('');
