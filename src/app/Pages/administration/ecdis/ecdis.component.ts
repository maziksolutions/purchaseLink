...
@Component({
  selector: 'app-ecdis',
  templateUrl: './ecdis.component.html',
  styleUrls: ['./ecdis.component.css']
})
export class ECDISComponent implements OnInit {
  ECDISForm: FormGroup; flag; pkey: number = 0;
  @ViewChild('searchInput') searchInput: ElementRef;
  displayedColumns: string[] = ['checkbox', 'model','maker','mobileNo','postCode','street'];
  ...
  get fm() { return this.ECDISForm.controls };
  loadRights(){
    this.userManagementService.checkAccessRight(administrationNavEnum.eCDIS).subscribe((response)=>{
if(response.status){
...
console.log(error);
  ...
  loadData(status: number) {
    ...
    this.vesselManagementService.getECDIS(status)
      .subscribe(response => {
        this.flag = status;
  ...
  onSubmit(form: any) {
    this.vesselManagementService.addECDIS(form.value)
      .subscribe(data => {
  ...
  Updatedata(id) {
    ...
    this.vesselManagementService.getECDISById(id)
      .subscribe((response) => {
        if (response.status) {
          this.ECDISForm.patchValue(response.data);
          this.pkey = response.data.ecdisId;
  ...
  DeleteData() {
    ...
    if (numSelected.length > 0) {
      Swal.fire({
        ...
        if (result.value) {
          this.vesselManagementService.archiveECDIS(numSelected).subscribe(result => {
  ...
  clear() {
    this.ECDISForm.reset();
    this.ECDISForm.controls.ecdisId.setValue(0);
  ...
  exportAsXLSX(data: any[]): void {
    ...
    this.exportExcelService.exportAsExcelFile(data, 'ECDIS', 'ECDIS');
  }
  ...
  exportLoadSheet() {
    ...
    if (numSelected.length > 0) {
      data = numSelected;
      data=data.map(item=>({
        model: item.model,maker:item.maker,street:item.street,postCode:item.postCode,mobileNo:item.mobileNo
       })); 
    }
    else
      data = [{ model: '',maker:'',street:'',postCode:'',mobileNo:'' }];
    this.exportExcelService.LoadSheet(data, 'ECDISLoadSheet', 'ECDIS Load Sheet',2);
  }
  ...
  openModal() {   
    const dialogRef = this.dialog.open(ImportDataComponent, {
      width: '500px',
      data:{modalTitle: "Import ECDIS",columncheck:"Type1",tablename:"tblECDIS",columname:"model"},
    });
