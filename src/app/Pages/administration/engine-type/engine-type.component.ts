...
@Component({
  selector: 'app-engine-type',
  templateUrl: './engine-type.component.html',
  styleUrls: ['./engine-type.component.css']
})
export class EngineTypeComponent implements OnInit {
  engineTypeForm: FormGroup; flag; pkey: number = 0;
  @ViewChild('searchInput') searchInput: ElementRef;
  displayedColumns: string[] = ['checkbox', 'engineType'];
  ...
  constructor(private fb: FormBuilder, public dialog: MatDialog,
    private exportExcelService: ExportExcelService,private router:Router, private swal: SwalToastService,
    private userManagementService: UserManagementService, private vesselManagementService:VesselManagementService) { }

  ngOnInit(): void {

    this.engineTypeForm = this.fb.group({
      engineTypeId: [0],
      engineTypeName: ['', [Validators.required]]
    });
    this.loadRights();
this.loadData(0);
  }
  get fm() { return this.engineTypeForm.controls };

  loadRights(){
    this.userManagementService.checkAccessRight(administrationNavEnum.engineType).subscribe((response)=>{
if(response.status){
...
console.log(error);
  ...
  loadData(status: number) {
    ...
    this.vesselManagementService.getEngineTypes(status)
      .subscribe(response => {
        this.flag = status;
  ...
  onSubmit(form: any) {
    this.vesselManagementService.addEngineType(form.value)
      .subscribe(data => {
  ...
  Updatedata(id) {
    ...
    this.vesselManagementService.getEngineTypeById(id)
      .subscribe((response) => {
        if (response.status) {
          this.engineTypeForm.patchValue(response.data);
          this.pkey = response.data.engineTypeId;
  ...
  DeleteData() {
    ...
    if (numSelected.length > 0) {
      Swal.fire({
        ...
        if (result.value) {
          this.vesselManagementService.archiveEngineType(numSelected).subscribe(result => {
  ...
  clear() {
    this.engineTypeForm.reset();
    this.engineTypeForm.controls.engineTypeId.setValue(0);
  ...
  exportLoadSheet() {
    ...
    if (numSelected.length > 0) {
      ...
      data = [{ engineTypeName: '' }];
    this.exportExcelService.LoadSheet(data, 'EngineTypeLoadSheet', 'Engine Type Load Sheet',1);
  }
  ...
  openModal() {   
    const dialogRef = this.dialog.open(ImportDataComponent, {
      width: '500px',
      data:{modalTitle: "Import Engine Type",columncheck:"Type1",tablename:"tblEngineType",columname:"EngineTypeName"},
    });
