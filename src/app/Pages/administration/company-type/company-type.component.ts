...
export class CompanyTypeComponent implements OnInit {
  companyTypeForm: FormGroup; flag; pkey: number = 0;
  displayedColumns: string[] = ['checkbox', 'companyTypeName'];
  ...
  get fm() { return this.companyTypeForm.controls };

  loadRights(){
    this.userManagementService.checkAccessRight(administrationNavEnum.companyType).subscribe((response)=>{
if(response.status){
...
console.log(error);
  ...
  loadData(status: number) {
    ...
    this.userManagementService.getCompanyType(status)
      .subscribe(response => {
        this.flag = status;
  ...
  onSubmit(form: any) {
    this.userManagementService.addCompanyType(form.value)
      .subscribe(data => {
  ...
  Updatedata(id) {
    ...
    this.userManagementService.getCompanyTypeById(id)
      .subscribe((response) => {
        if (response.status) {
          this.companyTypeForm.patchValue(response.data);
          this.pkey = response.data.companyTypeId;
  ...
  DeleteData() {
    ...
    if (numSelected.length > 0) {
      Swal.fire({
        ...
        if (result.value) {
          this.userManagementService.archiveCompanyType(numSelected).subscribe(result => {
  ...
  clear() {
    ...
    this.companyTypeForm.controls.companyTypeId.setValue(0);
  ...
  exportLoadSheet() {
    ...
    this.exportExcelService.LoadSheet(data, 'CompanyTypeLoadSheet', 'Company Type Load Sheet',1);
  }
